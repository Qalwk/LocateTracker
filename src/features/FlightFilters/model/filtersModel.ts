...
export const setFilters = createEvent<
  Partial<{
    flightId: string;
...
export const $flights = createStore<any[]>([]).on(
  setFlights,
  (_, flights) => flights,
);
  ...
  (flights, filters) =>
    flights.filter((flight) => {
      ...
      const matchCompany = filters.flightCompany
        ? flight.airline
            .toLowerCase()
            .includes(filters.flightCompany.toLowerCase())
        : true;
      const matchFrom = filters.flightFrom
        ? flight.from.city
            .toLowerCase()
            .includes(filters.flightFrom.toLowerCase()) ||
          flight.from.iata
            .toLowerCase()
            .includes(filters.flightFrom.toLowerCase())
        : true;
      const matchTo = filters.flightTo
        ? flight.to.city
            .toLowerCase()
            .includes(filters.flightTo.toLowerCase()) ||
